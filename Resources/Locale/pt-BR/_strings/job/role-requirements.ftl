...
role-timer-overall-too-high  = Excedido em [color=yellow]{ $time }[/color] tempo total de reprodução. (Escolhendo uma função de estudante?)
role-timer-role-insufficient  = Excedido em [color=yellow]{ $time }[/color] tempo para [color={ $departmentColor }]{ $job }[/color]. (A função de novato não está disponível?)
